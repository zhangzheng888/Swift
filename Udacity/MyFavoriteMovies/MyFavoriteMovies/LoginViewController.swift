...
class LoginViewController: UIViewController {
    ...
    private func getRequestToken() {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            do {
                ...
                displayError("Could not the data as JSON")
                return
            }
            
            if let _ = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int {
                displayError("TMDB returned an error status code")
                return
            }
            
            guard let requestToken = parsedResult[Constants.TMDBResponseKeys.RequestToken] as? String else {
                displayError("There is no Request Token")
                return
            }
    ...
    private func loginWithToken(_ requestToken: String) {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            guard (error == nil) else {
                displayError("The login did not work")
                return
            }
            ...
            guard let data = data else {
                displayError("No data returned in the request")
                return
            }
        ...
        /* 5. Parse the data */
            ...
            if let _ = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int {
                displayError("The request returned an error status code")
            }
            
            guard let success = parsedResult[Constants.TMDBResponseKeys.Success] as? Bool, success == true else {
                displayError("The request was not successful")
                return
            }
        /* 6. Use the data! */
    ...
    private func getSessionID(_ requestToken: String) {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            func displayError(_ error: String) {
                ...
                performUIUpdatesOnMain {
                    self.setUIEnabled(true)
                    self.debugTextLabel.text = "Failed to obtain SessionID"
                }
                
            }
            
            guard (error == nil) else {
                displayError("There is an error with the request")
                return
            }
            
            guard let statusCode = (response as? HTTPURLResponse)?.statusCode, statusCode >= 200 && statusCode <= 299 else {
                displayError("Status code is not 2xx!")
                return
            }
        ...
        /* 5. Parse the data */
            ...
            do {
                ...
                displayError("The result can not be parsed into JSON")
                return
            }
            ...
            guard let sessionID = parsedResult[Constants.TMDBResponseKeys.SessionID] as? String else {
                displayError("SessionID could not be retrieved")
                return
            }
        /* 6. Use the data! */
